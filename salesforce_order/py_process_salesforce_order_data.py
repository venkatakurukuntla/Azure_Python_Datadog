#######################################################################################################################
## File Name: py_process_salesforce_order_data.py                                                                     #
## Description: Python Script for transform and load of salesforce order object.                                      #
##                                                                                                                    #
...
def get_latest_order_timestamp_blob(container_name,cs):
    blob_name1="order/data/input_archive/order_"
    def format_order_number(order_number):
                return f'{int(order_number):08}'
...
def process_failure_data(connection_string, container_name,latest_blob_name):
    ...
    blob_name1 = "order/data/order_"
    blob_name2 = "order/data/order_hist.json"
    blob_name3="order/data/timestamp.json"
    order_hist_json_path_in_blob = "order/data/order_hist.json"
    try:
        msg = ''
        ...
        for blob_name1 in csv_blobs:
            ...
            if blob_client1.exists() and blob_client2.exists() and blob_client3.exists() :
                ...
                csv_df['OrderNumber'] = csv_df['OrderNumber'].apply(format_order_number)
                csv_df['WorkdayContractExternalID__c'] = csv_df['WorkdayContractExternalID__c'].apply(format_order_number)
                data_list = csv_df.to_dict(orient='records')
                json_ordernumbers = set(item["OrderNumber"] for item in json_data)
                filtered_csv_df = csv_df[~csv_df["OrderNumber"].isin(json_ordernumbers)]
                ...
                container_client = blob_service_client.get_container_client(container_name)
                blob_client_order_json = container_client.get_blob_client(order_hist_json_path_in_blob)
                ...
                if new_archive_blob_client.exists():
                    ...
                    csv_df1['OrderNumber'] = csv_df1['OrderNumber'].apply(format_order_number)
                    are_equal = csv_df['OrderNumber'].equals(csv_df1['OrderNumber'])
...
def fix_processing(connection_string, container_name):
    ...
    blob_name1 = "order/data/order_"
    blob_name2 = "order/data/order_hist.json"
    blob_name3 = "order/data/order_fix_hist.json"
    blob_name4="order/data/timestamp.json"
    order_hist_json_path_in_blob2 = "order/data/order_fix_hist.json"
    ...
    try:
        ...
        for blob_name1 in csv_blobs:
            ...
            if blob_client1.exists() and blob_client2.exists() and blob_client3.exists():
                ...
                json_ordernumbers2 = set(item["OrderNumber"] for item in json_data2)
                filtered_csv_df = csv_df[~csv_df["OrderNumber"].isin(json_ordernumbers2)]
                filtered_json_data2 = [item for item in json_data2 if item["OrderNumber"] not in [record["OrderNumber"] for record in datalist]]

                json_ordernumbers3 = set(item["OrderNumber"] for item in json_data3)
                filtered_json_data3 = [item for item in filtered_json_data2 if item["OrderNumber"] not in json_ordernumbers3]
                ...
                container_client = blob_service_client.get_container_client(container_name)
                blob_client_order_json2f = container_client.get_blob_client(order_hist_json_path_in_blob2)
...
def order(json_data,get_configvalues,cs,msg):
  if msg =='success':
    updated_data = []
    for item in json_data:
        ...
        record={
            ...
            "custom" : {
                ...
                "external_id" : item['WorkdayContractExternalID__c'],
                "status": item['Status'],
                "start_date": item['EffectiveDate'][:23],
                "type": item['Type'],
                "amount_currency" : item['CurrencyIsoCode'],
                "amount": item['TotalAmount'],
                "created_date": item['CreatedDate'][:23],
                "sync_to_wd": item['Sync_Customer_to_WD__c'],
    ...
    blob_service_client = BlobServiceClient.from_connection_string(cs)
    container_client = blob_service_client.get_container_client('salesforce')
    blob_client = container_client.get_blob_client('order/data/output_archive/')
    blob_name = f"order/data/output_archive/order_fail_{timestamp_str}.json"
    blob_client = container_client.get_blob_client(blob_name)
...
def push_order(get_configvalues,Api,updated_data_order):
...
def Orderfix(filtered_json_data3,get_configvalues,cs,msg):
  if msg =='success':
    updated_data = []
    ...
    container_client = blob_service_client.get_container_client('salesforce')
    blob_client = container_client.get_blob_client('order/data/output_archive/')
    blob_name = f"order/data/output_archive/order_fix_{timestamp_str}.json"
    blob_client = container_client.get_blob_client(blob_name)
    blob_client.upload_blob(data_new1,overwrite=True)
    return "success",data_new
  else:
...
def push_order_fix(get_configvalues,Api,Order_fix):
    url = get_configvalues["dd_url"]
    headers = {
    'Accept': 'application/json',
    'DD-API-KEY': Api,
    ...
    response = requests.post(url, headers=headers, json=Order_fix)
    print(response)

def moving_files(cs):
    container_name = "salesforce"
    source_folder1 = "order/data/order_"
    input_archive_folder= "order/data/input_archive/order_"
    blob_service_client = BlobServiceClient.from_connection_string(cs)
    container_client = blob_service_client.get_container_client(container_name)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    container_name = "salesforce"
    blob_name="order/logic/config.json"
    ...
    if msg=="Done":
        ...
        try:
                ...
                order_msg,updated_data_order = order(filtered_data_list,get_configvalues,cs,msg)
                contract_fix_msg,Order_fix=Orderfix(filtered_json_data3,get_configvalues,cs,msg)
                v_result2 = f_job_stats_gathering("pl_transform_salesforce_order_data", "salesforce", "order/pipeline", "Success", v_job_start_time,cs,Api)
        except Exception as e:
                v_result3 = f_job_stats_gathering("pl_transform_salesforce_order_data", "salesforce", "order/pipeline", "Failure", v_job_start_time,cs,Api)     
        try:
                ...
                pushorder=push_order(get_configvalues,Api,updated_data_order)
                push_orderfix=push_order_fix(get_configvalues,Api,Order_fix)
                v_result2 = f_job_stats_gathering("pl_load_salesforce_order_data", "salesforce", "order/pipeline", "Success", v_job_start_time1,cs,Api)     
        except Exception as e:
                v_result3 = f_job_stats_gathering("pl_load_salesforce_order_data", "salesforce", "order/pipeline", "Failure", v_job_start_time1,cs,Api)
